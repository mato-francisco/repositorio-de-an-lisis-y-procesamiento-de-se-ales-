...
ffx_1=(1/(10*N))*np.fft.fft(x_1,n=10*N,axis=0)
absffx_1=np.abs(ffx_1)
ff=np.arange(N)*(fs/(N))

ffx_2=(1/(10*N))*np.fft.fft(x_1*hamming,n=10*N,axis=0)
absffx_1=np.abs(ffx_1)
ff=np.arange(N)*(fs/(N))

ffx_3=(1/(10*N))*np.fft.fft(x_1*flatop,n=10*N,axis=0)
absffx_1=np.abs(ffx_1)
ff=np.arange(N)*(fs/(N))

ffx_4=(1/(10*N))*np.fft.fft(x_1*blackmanharris,n=10*N,axis=0)
absffx_1=np.abs(ffx_1)
ff=np.arange(N)*(fs/(N))


estimador_1=np.argmax(np.abs(ffx_1[0:5000,:]),axis=0)/10
var_rectangular=np.var(estimador_1)
media_rectangular=np.mean(estimador_1)

estimador_2=np.argmax(np.abs(ffx_2[0:5000,:]),axis=0)/10
var_hamming=np.var(estimador_2)
media_hamming=np.mean(estimador_2)

estimador_3=np.argmax(np.abs(ffx_3[0:5000,:]),axis=0)/10
var_flatop=np.var(estimador_3)
media_flatop=np.mean(estimador_3)

estimador_4=np.argmax(np.abs(ffx_4[0:5000,:]),axis=0)/10
